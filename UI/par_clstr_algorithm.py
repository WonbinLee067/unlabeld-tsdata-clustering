...
def param_kmeans():
    params = html.Div([
            ...
            cc.num_input_layout('try_n_init', min=1, max=50, init_value=10),
            html.Label('Kmeans가 알고리즘 안에 반복되는 최대 횟수'),
            cc.num_input_layout('try_n_kmeans', min=10, max=1000, init_value=300),
            html.Label('중심 랜덤으로 지정하기'),
            cc.switch_layout('random_center', label="랜덤 사용"),
        ])
    return params
